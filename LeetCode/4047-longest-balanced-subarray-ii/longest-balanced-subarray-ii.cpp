...
public:
    ...
    int findFirst(int node, int segLeft, int segRight, int limit) {
        if (segLeft > limit) return -1;
        if (seg[node].mn > 0 || seg[node].mx < 0) return -1;
        ...
        if (mid < limit) return findFirst(node << 1 | 1, mid + 1, segRight, limit);
    ...
    int longestBalanced(vector<int>& nums) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (start != -1) maxLen = std::max(maxLen, i - start + 1);
        }
