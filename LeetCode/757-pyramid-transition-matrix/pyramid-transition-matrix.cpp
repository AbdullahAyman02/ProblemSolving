class Solution {
public:
    vector<string> gen_neighbor(const string& node, unordered_map<string, unordered_set<char>>& mp) {
        vector<string> res{""};
        int n = node.size();
        ...
        for (int i = 1; i < n; i++) {
            ...
            for (const string& a : res) {
                for (char c : mp[key]) newRes.push_back(a + c);
            }
            res.swap(newRes);
        }
    ...
    bool pyramidTransition(string bottom, vector<string>& allowed) {
        ...
        function<bool(const string&)> dfs = [&](const string& node) -> bool {
            ...
            for (const string& nxt : gen_neighbor(node, mp)) if (dfs(nxt)) return true;
            visited.insert(node);
            return false;
        };
