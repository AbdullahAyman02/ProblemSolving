class Solution {
    ...
    long long solve(int i) {
        ...
        if (src[i] == tgt[i]) result = min(result, solve(i + 1));
        ...
        for (auto &p1 : mp) {
            ...
            for (auto &p2 : mp) {
                ...
                result = min(result, dist[u][v] + solve(i + len));
            }
        }
...
public:
    long long minimumCost(string source, string target, vector<string>& original, vector<string>& changed, vector<int>& cost) {
        ...
        for (int k = 0; k < cnt; k++) for (int i = 0; i < cnt; i++) for (int j = 0; j < cnt; j++) dist[i][j] = min(dist[i][j], dist[i][k] + dist[k][j]);
